...
def interp_on_structured(lon_st:np.ndarray, lat_st:np.ndarray, files: List[pd.DataFrame], fileName: List[str], nPt_int: int, fileDest: str, perc: int):
    ...
    if not (os.path.exists(depth_path) & os.path.exists(percentage_path)):
        ...
        lonHD=np.linspace(lonbins[0],lonbins[-1],nPt_int*(len(lonbins)-1))                                                     # CL
        latHD=np.linspace(latbins[0],latbins[-1],nPt_int*(len(latbins)-1))                                                     # CL
        ...
        for ii in np.arange(len(lonbins)-1):
            for jj in np.arange(len(latbins)-1):
                ...
                for nfile in np.arange(len(files)):     # Loop over unstructured data files 
                    ...
                    depthHD[nfile,jj*nRef:(jj+1)*nRef,ii*nRef:(ii+1)*nRef] = loc_depth[:,:]                                     # CL 
                    occurHD[nfile,jj*nRef:(jj+1)*nRef,ii*nRef:(ii+1)*nRef] = loc_occ[:,:]                                       # CL
                ...
                loc_mask_sum = loc_bool_occs[0]                                               ## CL
                ...
                for nloc in np.arange(1,len(files)):
                    ...
                    if nloc ==1 and loc_bool_occs[0].sum()>0:                                              ## CL : added if(nloc ==1)
                    ...
                    loc_mask_sum=loc_mask_sum+loc_bool_occs[nloc]                             ## CL : alternative
                loc_depth=loc_depth/loc_mask_sum                                              ## CL : alternative
                if present_in_file !=0:                                                       ## CL : alternative
                  global_depth[jj][ii] = np.sum(loc_depth)/(nRef)**2)                         ## CL : alternative
                else:                                                                         ## CL : alternative
               ...
               #    global_depth[jj][ii] = (np.nansum(loc_depth)/(nRef)**2) / present_in_file ## CL : wrong
                                                                                              ## CL : wrong 
               #else:                                                                         ## CL : wrong 
