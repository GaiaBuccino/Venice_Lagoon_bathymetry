...
def cleaning(bathy: xr.DataArray, percentage: int, coverage_percentage: np.ndarray, channel:bool = True): 
    ...
    bathy.loc[bathy.latitude[26],bathy.longitude[47]] = val
    ...
    if not (os.path.exists(depth_path) & os.path.exists(percentage_path)):
        ...
        for ii in np.arange(len(lonbins)-1):
            for jj in np.arange(len(latbins)-1):
                ...
                #if present_in_file !=0:                                                       ## CL : wrong
                #    global_depth[jj][ii] = (np.nansum(ref_depth)/(nRef)**2) / present_in_file ## CL : wrong
                                                                                              ## CL : wrong 
                #else:                                                                         ## CL : wrong 
                #    global_depth[jj][ii] = nan                                                ## CL : wrong 
                                                                                              ## CL : wrong 
        ...
        # #filtered_depth = xr.DataArray(filtered_depth)
        # filtered_depth = cleaning(filtered_depth)
...
lagoon_bathy = original_bathy.sel(longitude = slice(12.22265625, 12.691406250), latitude= slice(45.12109375,45.589843750))      # Coordinates of the area occupied by the lagoon 61x61
...
original_files =  ['Bathy_2013_coarsed']   #['Bathy_2003CORILA',  # files with unstructured data to be averaged on the grid
...
for pt in nPt:
    ...
    plt.pcolormesh(filtered_depth, vmax = -0.001)
    plt.colorbar()
    plt.savefig(fileDest + f'Filtered_depths_{minimal_percentage}perc_OUT_FUNC_{pt}', bbox_inches = 'tight', pad_inches = 0)
    plt.close() 
   
    plt.figure()
    plt.pcolormesh(original_bathy, vmax = -0.001)
    plt.colorbar()
    plt.savefig(fileDest + f'bathy-PRE_outSUM_{minimal_percentage}perc__{pt}', bbox_inches = 'tight', pad_inches = 0)
    plt.close() 
    ...
    plt.colorbar()
    plt.savefig(fileDest + f'BATHYMETRY_{minimal_percentage}perc_OUT_FUNC_{pt}', bbox_inches = 'tight', pad_inches = 0)
    plt.close() 
    ...
    bathymetry.to_netcdf(fileDest + f'NAS_lagoon_bathymetry_{pt-1}ref_{minimal_percentage}perc.nc')
    bathymetry.values.astype('f4').tofile(fileDest + f'NAS_lagoon_bathymetry_{pt-1}ref_{minimal_percentage}perc.bin')

    fig = plt.figure()
    bathymetry.plot(vmin = -1.5, vmax =-0.001)

    #plt.gcf().set_size_inches(40, 24)
    plt.savefig(fileDest + f"NAS_lagoon_bathymetry_{pt-1}ref_{minimal_percentage}perc", bbox_inches = 'tight', pad_inches = 0)
...
# TOGLIERE AUTOMATICAMENTE L'INTERVALLO 0-20%